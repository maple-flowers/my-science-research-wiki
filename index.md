...
*文献鸟（Stork）自动推送写入 `raw/文献日报/`，由 [[SCHEMA#摄入源|摄入源规范]] 与 `stork_daily.py` 维护（该脚本当前不存在、待重建）。当前目录暂无已归档日报。*
...
- `output/`：近期维护产出的报告与卡片（如 figures 双链检查/修复报告），非知识库正式层。
- `temp/`：近期维护的中间产物（JSON 清单、fix_backup 备份目录），非知识库正式层。
