...
*   **关键特征**：以流程图形式展示了各光学和电子元件的连接关系。_（可设计为流程图：激光器(L) → 声光调制器(AOM，用于选单和控制开关) → 高反镜 → 二向色镜 → 物镜 → 聚焦到样品台(SH)。样品台由3D
*   **来源**：[[../papers/Perugu2024morphology]]

*   **关键特征**：(a) 介电常数（ε'，即能量存储）随频率变化图；(b) 介电损耗（ε"，即能量耗散）随频率变化图；(c) 交流电导率（σ_ac）
*   **来源**：[[../papers/Perugu2024morphology]]

*   **关键特征**：(a) 磁滞回线（M-H Loop），展示了磁化强
*   **来源**：[[../papers/Şahin2009probe]]

*   **关键特征**：比较了两种前向探测器接受度下的有效 γγ 光度（dL_{γγ}/dW），纵轴单位为 GeV^{-1}，横轴为双光子系统不变质量 W（单位：GeV）。（LAS）。控制。）_
...
*   **关键特征**：比较了两种前向探测器接受度下的有效 γγ 光度（dL_{γγ}/dW），纵轴单位为 GeV^{-1}，横轴为双光子系统不变质量 W（单位：GeV）。（LAS）。控制。）_
> 收录薄膜介电函数与光学带隙、非线性光学与 Z 扫描、介电函数第一性原理计算、以及光电器件与光谱应用相关的图表与公式。
