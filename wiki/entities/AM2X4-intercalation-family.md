---
tags: [entity, 2D-materials, berry-phase, density-functional-theory, magnetoelectric-coupling, multiferroicity]
title: AM2X4-intercalation-family
type: entity
status: developing
year: 2024
papers: [zhaoRealization2DMultiferroic2024]
updated: 2026-08-18
---

# AM2X4-intercalation-family

摘要新的二维多铁性材料的发现具有实现信息存储和逻辑器件的潜力。

## 👵 太奶导读

乖孙，这一条要讲的核心对象是「摘要新的二维多铁性材料的发现具有实现信息存储和逻辑器件的潜力」。
这篇论文为它提供了关键证据。

## 🧩 核心内容与机制 (Core Content)


## 📚 相关论文 (Related Papers)

- [[../papers/zhaoRealization2DMultiferroic2024]]：为本文档提供核心证据。

## 🔗 关联概念与实体 (Related)

- [[../concepts/berry-phase|berry-phase]]
- [[../concepts/density-functional-theory|density-functional-theory]]
- [[../concepts/magnetoelectric-coupling|magnetoelectric-coupling]]
- [[../concepts/multiferroicity|multiferroicity]]
- [[../concepts/polarization-switching|polarization-switching]]
- [[../concepts/spin-orbit-coupling|spin-orbit-coupling]]
- [[../concepts/topological-defects|topological-defects]]
- [[../entities/CrTe2|CrTe2]]
- [[../entities/In2Se3|In2Se3]]
- [[../entities/TMDs|TMDs]]
- [[../entities/VASP|VASP]]
- [[../entities/WTe2|WTe2]]
