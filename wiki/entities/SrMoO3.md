...
**SrMoO3** 是一种钙钛矿氧化物材料。在二维极限下，单层 SrMoO3 被预测为一种具有高 Néel 温度（$T_N \approx 315\text{ K}$）的二维反铁磁材料，其电子态可以通过应变诱导的相变在半导体和自旋极化金属之间切换 [[../../raw/note/zhongHighthroughputExfoliationMultiferroic2025|Zhong et al. 2025]]。

## 1. 高温反铁磁性 (High-Temperature Antiferromagnetism)

在作者筛选出的 35 种稳定 $ABO_3$ 单层中，SrMoO3 展现出最高的磁转变温度：
- **磁基态**：表现为 Néel 型反铁磁（AFM）序。
- **转变温度**：预测的 Néel 温度 $T_N = 315\text{ K}$，超过了室温（300 K），这使其在常温自旋电子器件应用中极具潜力。
- **物理起源**：源于强的层内超交换相互作用（Super-exchange），磁性原子（Mo）通过氧桥介导磁耦合。

## 2. 相锁定切换 (Phase-interlocked Switching)

单层 SrMoO3 的电子结构与晶格对称性高度锁定：
...
| **剥离能 ($E_{exf}$)** | 低                    | 符合高通量剥离判据 |
| **带隙 ($E_g$)**      | $0.19\text{ eV}$     | $P4mm$ 相  |
| **自旋极化率**           | 100%                 | $Pc$ 金属相下 |
