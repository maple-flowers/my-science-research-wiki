---
tags: [entity]
title: 'CuInP2S6'
type: entity
status: developing
papers: ['huangTwodimensionalIn2Se3Rising2022', 'cuiIntercorrelatedInplaneOutofplane2018a', 'guoAdvancesTwodimensionalFerroelectric2025', 'kaurRecentAdvancesTheoretical2025a', 'gaoStrainEngineeringFerroelectric2024', 'FerroelectricityMultiferroicityAtomic2023', 'zhangEmergingFrontiersTwodimensional2025', 'liPhaseTransitions2D2021', 'guanRecentProgressTwoDimensional2020', 'neumayerCompetingPolarPhases2025']
updated: 2026-08-18
---

# CuInP2S6

CuInP₂S₆（CIPS）是**室温铁电的层状范德华硫代磷酸盐材料**，居里温度约 315 K，Cu 离子在层间双势阱中位移产生面外极化。CIPS 兼具可机械剥离、室温铁电、离子迁移与铁弹行为，是二维铁电、铁电隧道结、忆阻器与离子-电子耦合器件研究的重要平台（本库二维铁电相关论文的核心材料）。

## 👵 太奶导读

太奶啊，CuInP₂S₆ 是一层能"记住电方向"（铁电）的超薄晶体，而且在室温就能用，很实用。它的秘密在于层间的铜离子能在两个位置之间"搬家"：大部分搬到一边就产生极化，还能被电场掰过来。因为能撕成几层薄膜、又能在常温下工作，它成了做二维铁电存储、忆阻器的明星材料。

## 🧩 核心内容与机制 (Core Content)

- **结构与铁电**：层状范德华结构，Cu⁺ 离子在层间双势阱位移形成面外极化；T_C ≈ 315 K（本库 CIPS 铁电论文）。
- **离子迁移**：Cu 离子迁移导致极化动力学复杂与离子电导，可用于忆阻/离子器件（本库 CIPS 忆阻与离子迁移论文）。
- **二维铁电器件**：超薄 CIPS 铁电场效应、铁电隧道结（ferroelectric-tunnel-junction）与非易失存储（本库二维铁电器件论文）。
- **调控**：厚度、电场、压力与异质结构调控极化与相变（本库 CIPS 相变论文）。
- **表征**：PFM（见 entities/PFM）、SHG、变温介电与 XRD 表征。

## 🔗 关联概念与实体 (Related Concepts & Entities)

- [[../concepts/2d-materials|二维材料]]：CIPS 的载体平台。
- [[../concepts/ferroelectricity|铁电性]]：CIPS 的核心功能。
- [[../concepts/curie-temperature|居里温度]]：CIPS 的铁电转变点。
- [[../entities/CuCrP2S6|CuCrP₂S₆]]：同族磁性硫代磷酸盐。
- [[../concepts/ferroelectric-tunnel-junction|铁电隧道结]]：CIPS 器件应用。

## 📚 相关论文 (Related Papers)

- [[../papers/huangTwodimensionalIn2Se3Rising2022]] — Two-dimensional In2Se3: A rising advanced material for ferroelectric data storage
- [[../papers/cuiIntercorrelatedInplaneOutofplane2018a]] — Intercorrelated In-Plane and Out-of-Plane Ferroelectricity in Ultrathin Two-Dimensional Layered Semiconductor In2Se3
- [[../papers/guoAdvancesTwodimensionalFerroelectric2025]] — Advances in two-dimensional ferroelectric materials
- [[../papers/kaurRecentAdvancesTheoretical2025a]] — Recent advances in theoretical investigations of sliding ferroelectricity in layered and van der Waals two-dimensional materials
- [[../papers/gaoStrainEngineeringFerroelectric2024]] — Strain engineering of ferroelectric polarization and domain in the two-dimensional multiferroic semiconductor
- [[../papers/FerroelectricityMultiferroicityAtomic2023]] — Ferroelectricity and multiferroicity down to the atomic thickness
- [[../papers/zhangEmergingFrontiersTwodimensional2025]] — Emerging frontiers in two-dimensional sliding ferroelectrics
- [[../papers/liPhaseTransitions2D2021]] — Phase transitions in 2D materials
- [[../papers/guanRecentProgressTwoDimensional2020]] — Recent Progress in Two‐Dimensional Ferroelectric Materials
- [[../papers/neumayerCompetingPolarPhases2025]] — Competing polar phases in 2D ferroelectric transition metal thio- and selenophosphates

## 🏷️ 专业名词别名

- `cips-cu-in-p2s6`（entities）
