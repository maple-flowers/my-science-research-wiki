---
tags: [concept, 2D-materials, charge-density-wave, density-functional-theory, moire-superlattice, strain-engineering]
title: cdw-blends-displacements
type: concept
status: developing
year: 2024
papers: [cossuStackingChargedensityWaves2024]
updated: 2026-08-18
---

# cdw-blends-displacements

我们利用电子结构计算来研究双层膜中的电荷密度波和周期晶格畸变2H负极NbSe公司2我们证明了垂直叠加可以产生各种各样的图案，这些图案可能会降低电荷密度波的对称性1H负极NbSe公司2单层膜。

## 👵 太奶导读

乖孙，这一条要讲的核心对象是「我们利用电子结构计算来研究双层膜中的电荷密度波和周期晶格畸变2H负极NbSe公司2我们证明了垂直叠加可以产生各种各样的图案，这些图案可能会降低电荷密度波的对称性1H负极NbSe公司2单层膜」。
这篇论文为它提供了关键证据。

## 🧩 核心内容与机制 (Core Content)


## 📚 相关论文 (Related Papers)

- [[../papers/cossuStackingChargedensityWaves2024]]：为本文档提供核心证据。

## 🔗 关联概念与实体 (Related)

- [[../concepts/charge-density-wave|charge-density-wave]]
- [[../concepts/density-functional-theory|density-functional-theory]]
- [[../concepts/moire-superlattice|moire-superlattice]]
- [[../concepts/strain-engineering|strain-engineering]]
- [[../concepts/interlayer-stacking|interlayer-stacking]]
- [[../concepts/geometric-structure-factor|geometric-structure-factor]]
- [[../concepts/pseudogap|pseudogap]]
- [[../concepts/vdw-correction|vdw-correction]]
- [[../concepts/peierls-instability|peierls-instability]]
- [[../entities/TMDs|TMDs]]
- [[../entities/VASP|VASP]]
- [[../entities/Wannier90|Wannier90]]
- [[../entities/1T-TaS2|1T-TaS2]]
- [[../entities/NbSe2|NbSe2]]
