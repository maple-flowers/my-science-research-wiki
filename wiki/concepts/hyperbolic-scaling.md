---
tags: [concept, density-functional-theory, polarization-switching, strain-engineering, topological-defects, kittels-law]
title: hyperbolic-scaling
type: concept
status: developing
year: 2023
papers: [gongAbsenceCriticalThickness2023]
updated: 2026-08-18
---

# hyperbolic-scaling

摘要极域周期(d)在铁电材料中，人们通常认为它与相应的薄膜厚度成比例(h)，遵循经典的基特尔定律d$\ sqrt{h}$h在这里，我们不仅观察到这种关系在极性skyrmions的情况下是失败的，在极性skyrmions的情况下，周期几乎收缩到一个恒定值，甚至经历了一个轻微的增加，而且还发现skyrmions在[（PbTiO3)2/（SrTiO3)2]10超薄超晶格。

## 👵 太奶导读

乖孙，这一条要讲的核心对象是「摘要极域周期(d)在铁电材料中，人们通常认为它与相应的薄膜厚度成比例(h)，遵循经典的基特尔定律d$\ sqrt{h}$h在这里，我们不仅观察到这种关系在极性skyrmions的情况下是失败的，在极性skyrmions的情况下，周期几乎收缩到一个恒定值，甚至经历了一个轻微的增加，而且还发现skyrmions在[（PbTiO3)2/（SrTiO3)2]10超薄超晶格」。
这篇论文为它提供了关键证据。

## 🧩 核心内容与机制 (Core Content)


## 📚 相关论文 (Related Papers)

- [[../papers/gongAbsenceCriticalThickness2023]]：为本文档提供核心证据。

## 🔗 关联概念与实体 (Related)

- [[../concepts/density-functional-theory|density-functional-theory]]
- [[../concepts/polarization-switching|polarization-switching]]
- [[../concepts/strain-engineering|strain-engineering]]
- [[../concepts/topological-defects|topological-defects]]
- [[../concepts/polar-vortex|polar-vortex]]
- [[../concepts/depolarization-field|depolarization-field]]
- [[../concepts/polar-skyrmion|polar-skyrmion]]
- [[../concepts/critical-thickness-ferroelectric|critical-thickness-ferroelectric]]
- [[../concepts/phase-field-modeling|phase-field-modeling]]
- [[../concepts/topological-charge|topological-charge]]
- [[../entities/VASP|VASP]]
- [[../entities/SrTiO3|SrTiO3]]
- [[../entities/PbTiO3|PbTiO3]]
- [[../entities/LSMO|LSMO]]
