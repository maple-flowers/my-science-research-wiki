---
tags: [concept]
title: 'lone-pair-electrons'
type: concept
status: developing
papers: ['Kim2008effect', 'RecentAdvancesGrowth2025', 'bhowalPolarMetalsPrinciples2023b', 'caiFerroelectricitydrivenStrainmediatedMagnetoelectric2025', 'cheongMultiferroicsMagneticTwist2007a', 'fiebigEvolutionMultiferroics2016', 'hillWhyAreThere2000a', 'shenEmergenceMultipleFerroelectric2025', 'spaldinAdvancesMagnetoelectricMultiferroics2019', 'tangMultiferroicityTwodimensionalVan2025']
updated: 2026-08-18
---

# lone-pair-electrons

> [!warning] 本页内容待重写（贡献句部分）
> 本页「相关论文」中部分条目的贡献句为占位或缺失，尚未逐篇核实该论文对本条目的具体贡献。
> 太奶导读与正文可参考。（标记于 2026-08-21）


孤对电子（lone-pair electrons）指**未参与成键、独占一个轨道的价电子对**。在含 Bi³⁺、Pb²⁺、Sn²⁺、Se⁴⁺、I⁻ 等的化合物中，立体化学活性孤对（stereochemically active lone pair）驱动非中心对称畸变，是众多铁电、压电、非线性光学与拓扑材料的结构与功能起源。

## 👵 太奶导读

太奶啊，原子外面有电子"占位"成键，但总有些电子"单着"、不跟别人成键，自己占一个窝——这就是孤对电子。它们本来可能"藏着"（惰性），但在某些原子里很"活跃"，会"伸出一只脚"把周围的原子拱得不对称。这一拱，材料就不对称了（反演破缺），于是能铁电、能压电、能把光变倍频，还能搞出拓扑性质。

## 🧩 核心内容与机制 (Core Content)

- **立体化学活性孤对**：Bi³⁺/Pb²⁺/Sn²⁺ 等的 6s²/5s² 孤对在配位场中极化，偏离高对称位置，产生局部偶极（本库铁电、压电论文中的 5s²/6s² 机制）。
- **铁电与多铁**：孤对电子驱动的结构畸变是 BiFeO₃、BiMnO₃、SnTe 等铁电/多铁材料极化的来源（本库 BiFeO₃、SnTe 铁电论文）。
- **非线性光学**：孤对畸变增强二阶非线性（SHG），是红外非线性光学材料（如硒化物）的设计原则（本库 SHG 与非线性光学论文）。
- **拓扑与热电**：SnTe/GeTe 等孤对化合物的极性畸变与拓扑、热电性能耦合（本库 SnTe 相关论文）。
- **与反演破缺的关系**：孤对驱动的结构不对称是 inversion-symmetry-breaking 的重要微观途径。

## 🔗 关联概念与实体 (Related Concepts & Entities)

- [[../concepts/ferroelectricity|铁电性]]：孤对驱动极化。
- [[../concepts/inversion-symmetry-breaking|反演对称破缺]]：孤对效应的对称性后果。
- [[../concepts/second-harmonic-generation|二次谐波产生]]：孤对增强的光学响应。
- [[../entities/SnTe|SnTe]]：孤对驱动的铁电拓扑材料。

## 📚 相关论文 (Related Papers)

- [[../papers/Kim2008effect]] — Effect of epitaxial strain on ferroelectric polarization in multiferroic BiFeO3 films
- [[../papers/RecentAdvancesGrowth2025]] — Recent advances in growth, characterization, and application of two-dimensional multiferroic materials
- [[../papers/bhowalPolarMetalsPrinciples2023b]] — Polar Metals: Principles and Prospects
- [[../papers/caiFerroelectricitydrivenStrainmediatedMagnetoelectric2025]] — Ferroelectricity-driven strain-mediated magnetoelectric coupling in two-dimensional multiferroic heterostructure
- [[../papers/cheongMultiferroicsMagneticTwist2007a]] — Multiferroics: a magnetic twist for ferroelectricity
- [[../papers/fiebigEvolutionMultiferroics2016]] — The evolution of multiferroics
- [[../papers/hillWhyAreThere2000a]] — Why Are There so Few Magnetic Ferroelectrics?
- [[../papers/shenEmergenceMultipleFerroelectric2025]] — Emergence of multiple ferroelectric states in multilayer black phosphorus
- [[../papers/spaldinAdvancesMagnetoelectricMultiferroics2019]] — Advances in magnetoelectric multiferroics
- [[../papers/tangMultiferroicityTwodimensionalVan2025]] — Towards Multiferroicity in Two-Dimensional Van Der Waals Materials: Challenges and Opportunities
