...
## 📊 关键图表
笔记未附图片。manifest.json 中 figures 为空；原文仅有 Table I（ES/约束方法在虚/实变量下的方程对应关系）、Table II（各方法独立变量数：NVT 下 ES=6N+1、统计力学=6N、约束=6N-1；NPT 下 ES=6N+3、统计力学=6N+1、约束=6N-1，括号内为考虑动量与角动量守恒后的有效数）、Table III（参数 g 的取值：虚时间采样 ES=3N+1/约束=3N；实时间采样 ES=3N/约束=3N-1），均为文本/HTML 表格，无图片文件可嵌入。
