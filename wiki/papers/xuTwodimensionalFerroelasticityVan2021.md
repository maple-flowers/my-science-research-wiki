...
## ✏️ 可写入 Wiki 的要点
  ...
  5. 偏光成像利用线性二向色性：透射光强随起偏角 φ 呈余弦调制，三畴最大光强对应角相差 60°/120°，可快速无损确定畴取向。
  6. 铁弹畴切换：PET 柔性衬底两点弯曲施加单轴拉伸（ε=h/2R），≤0.5% 外应变即可触发；0.5%≤ε≤1.1% 区间 A 畴面积分数近似线性增长（斜率~1.35），外加应变几乎完全由 B→A 畴切换协调（理论上可产生~0.74% 垂直伸长）；卸载后残留新成核 A 畴，表现出铁弹迟滞；低/高应变区的偏离归因于畴壁钉扎。
  ...
  10. 普适性与展望：二维铁弹性应广泛存在于具有各向异性晶格畸变的 vdW 材料中，包括具 Peierls 畸变的 1T' TMDs（MoTe2、WTe2）和 SnTe 家族二维铁电体；β'-In2Se3 的热弹性转变有望成为二维形状记忆效应平台，并可通过应变/畴壁工程耦合调控量子自旋霍尔效应和面内铁电性，构筑 2D 多铁。
