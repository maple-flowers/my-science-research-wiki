...
- **元数据**：Tomoaki TSUJI、Tomohisa Kojima，2019，The Proceedings of the Materials and Mechanics Conference (JSME)，论文编号 OS0512，DOI: 10.1299/jsmemm.2019.OS0512
- **一句话**：用 PMT+脉冲 LED 的时间分辨装置测量静水载荷下 ML 材料的磷光衰减曲线，并以含吸收因子 γ（α、β）的指数模型修正标准单指数衰减，揭示自吸收对表观衰减的显著影响。
...
- **关键图表**：
  - 笔记未附图片（raw/figures/TSUJI2019phosphorescence/ 下仅有 manifest.json，figures 与 formulas 均为空数组；tables 仅为 Zotero 元数据表，非论文图表）。
  - 根据笔记可读文本，论文含一幅核心图：光强 I（0–4500）对时间 t（0–120 s），同时绘制 measured（实测散点）、exponential（单指数拟合）、absorption（吸收模型）三条曲线，并标注 α、β 两个过程参数。
...
- **可写入wiki的要点**：
  ...
  4. 吸收修正模型：在标准衰减基础上引入吸收/重吸收因子 γ，含 α、β 两个分支参数，用以描述发射光在样品内部被吸收后对表观衰减曲线的修正，称为"absorption model"。
  ...
  9. 本文为会议短文（OS0512，4 页量级），数据与模型较初步；具体 ML 材料的化学成分、陷阱深度、γ 随压力的定量关系等在可读笔记中未给出，第二步 wiki 写作时不应臆造。
