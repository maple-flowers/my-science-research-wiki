...
"研究背景": >-
  传统DOE制造方法（如光刻、电子束光刻）成本高、周期长且不灵活，难以快速测试不同光束形状。2PP技术虽有高精度和设计自由度，但此前制造的DOE尺寸较小，限制了其在高功率激光加工中的应用。本研究旨在填补这一空白，制造毫米级2PP-DOE，以验证其在高功率材料处理中的可行性。
"作者的问题意识": >-
...
"主要研究方法": >-
  1. **设计计算**：采用改进的棱镜与透镜算法，将透镜项替换为轴棱锥项，生成用于生成双贝塞尔光束的计算机生成全息图（CGH）相位图，并将其转换为STL文件。 2. **制造工艺**：使用双光子聚合（2PP）系统，通过将STL文件分割成7x7拼接网格、层厚0.1 μm的逐层扫描策略，以及倾斜校正补偿，耗时68.5小时完成制造。 3. **表征与测试**：使用光学显微镜和SEM观察形貌；用光束分析仪在远场（51 mm处）捕获光束轮廓以验证整形效果；使用高功率飞秒激光器（1030 nm）进行逐步升功率的损伤阈值测试。
"研究意义": >-
...
- **元数据**：Behlau, Marx, Zimmermann, Thüsing, Albini, Esen, Ostendorf，2025，Journal of Laser Micro/Nanoengineering, Vol. 20 No. 3，DOI: 10.2961/jlmn.2025.03.2008
- **一句话**：用双光子聚合（2PP）拼接制造出直径 3.5 mm 的衍射光学元件（DOE），将高斯光束整形为双环形（近场双贝塞尔）光束，并验证其可承受 22.8 W 平均功率 / 24.8 GW/cm² 峰值功率密度的飞秒激光辐照。

- **现有wiki双链**：
  - 年度 [[../write/2025]]
  - 项目 [[../projects/project-1-two-photon]]
  - 相关论文 [[../../raw/note/Unknown2025diffractive]]
  - （wiki 现有概念/实体条目集中在二维材料、多铁、铁电领域，本文涉及的光学/2PP 概念尚无对应条目）

- **新概念/实体建议**：
  - `two-photon-polymerization.md` — 双光子聚合（2PP），基于飞秒激光非线性双光子吸收的亚微米分辨率增材制造技术
  - `diffractive-optical-element.md` — 衍射光学元件（DOE），通过表面微结构相位调制实现光束分束、整形、聚焦的光学元件
  - `beam-shaping.md` — 光束整形，定制激光束空间强度分布以优化加工质量与效率
  - `computer-generated-hologram.md` — 计算机生成全息图（CGH），通过算法逆向计算所需相位分布而非记录真实物体干涉
  - `bessel-beam.md` — 贝塞尔光束，具有无衍射和自愈特性的传播不变光束，适合精密钻孔
  - `axicon.md` — 轴锥镜，锥形光学元件，将高斯光束转换为贝塞尔/环形光束
  - `staircase-effect.md` — 阶梯效应，逐层增材制造中离散层高导致的表面台阶状近似，引入高阶衍射伪影
  - `laser-damage-threshold.md` — 激光损伤阈值，光学元件可承受的最大激光功率/能量密度
  - `FemtoBond-4B.md` — 实体：Laserzentrum Hannover 生产的有机-无机杂化光刻胶，用于 2PP，折射率 n=1.55
  - `stitching.md` — 拼接技术，将大尺寸结构分割为物镜视场内的小块逐块制造以突破视场限制
