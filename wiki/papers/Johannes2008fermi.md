...
## ✏️ 可写入 Wiki 的要点
  ...
  10. Na 原子链（非链方向间距 38 Å，链内 Na–Na 3.34 Å）：钳制离子时纯电子 CDW 不稳定；允许沿 1D 弛豫也不二聚化（即使预置二聚化也回弹）；只有放开二维自由度才形成之字形畸变（间距 3.43 Å，角度 152°），晶胞加倍但 E_F 处无能隙——因为畸变是二维的而波函数是一维的，垂直方向积分使 BZ 边界矩阵元归零，紧束缚计算同样给出 k=±π/a 态简并。强制二聚化虽打开能隙导致能量上不稳定，故 Peierls 机制可被排除。
