...
- **元数据**：Yang Yang, Hongxiang Zong, Jun Sun, Xiangdong Ding et al.，2021，Advanced Materials 33(49), 2103469，DOI: 10.1002/adma.202103469
- **一句话**：通过自研机器学习势的大规模分子动力学模拟，首次揭示二维单层 GeSe 中本征波纹既能稳定高温相短程铁性序、提高冷却相变温度，又能将应力诱导畴翻转从协同雪崩式（幂律分布）转变为波纹驱动的局域随机过程（高斯分布）。
- **现有wiki双链**：
  - 概念 [[../concepts/2D-materials]]、[[../concepts/ferroelasticity]]、[[../concepts/multiferroicity]]、[[../concepts/strain-engineering]]、[[../concepts/density-functional-theory]]、[[../concepts/machine-learning-potential]]、[[../concepts/polarization-switching]]、[[../concepts/moire-superlattice]]
  - 实体 [[../entities/VASP]]、[[../entities/SnTe]]、[[../entities/In2Se3]]、[[../entities/WTe2]]、[[../entities/domain-wall]]
...
- **新概念/实体建议**：
  - `concepts/ripples.md` — 二维材料由面外弯曲柔性产生的本征褶皱结构缺陷，可量子化为 ripplocation，产生局域应变场
  - `concepts/ripple-engineering.md` — 通过基底/应变/激光图案化主动调控波纹密度、曲率、波长以设计铁性等物性的策略
  - `concepts/polar-nano-regions.md` — 高温顺电相中残留的短程极性/铁性有序微区，可作冷却异质形核点
  - `concepts/avalanche-dynamics.md` — 高度协同的畴翻转级联行为，应力降服从截断幂律分布，区别于独立随机的高斯过程
  - `concepts/flexoelectric-effect.md` — 应变梯度诱导极化的耦合效应，是波纹曲率与铁性序耦合的普遍物理基础
  - `concepts/atomistic-order-parameter.md` — 用相邻 Ge-Se 相对位移在局域切平面投影 Rp=(Δx,Δy) 同时刻画铁弹/铁电状态的微观序参量
  - `concepts/za-phonon-mode.md` — 二维材料面外弯曲声学声子模，主导波纹热激发，决定大尺度波纹的长寿命
  - `entities/GeSe.md` — 第四族单硫族化物，强耦合铁弹-铁电单层，四个能量等效畴变体
  - `entities/LAMMPS.md` — 大规模原子/分子 massively parallel simulator，本文 MD 模拟所用代码
  - `entities/CrI3.md` — 二维铁磁半导体，作者展望波纹-磁序耦合研究的候选体系
- **关键图表**：
  - ![图1 单层GeSe晶体结构、四畴变体、势能-温度相变曲线、晶格常数演化及原子级铁性序空间分布与角度关联函数](../../raw/figures/yangRipplingFerroicPhase2021/fig_1_9ERHR5CE.png)
...
- **可写入wiki的要点**：
  1. 单层 GeSe 属第四族单硫族化物 MX（M=Ge/Sn，X=S/Se/Te），热力学稳定相具有强耦合自发极化 P 与自发应变 η，存在四个能量等效畴变体 (ηx,±Px)、(ηy,±Py)，由 90° 畴壁分隔。
  2. 原子级铁性序参量定义为相邻 Ge-Se 相对位移矢量 R=R_Se−R_Ge 在局域切平面上的投影 Rp=(Δx,Δy)，单一参量同时刻画铁弹与铁电状态，可作空间/时间关联分析。
  3. 加热-冷却 MD 循环（0.77 K/ps）测得 GeSe 单层相变温度：加热约 310 K、冷却约 260 K，呈弱速率依赖的滞后；高温相 x/y 晶格常数趋同，自发应变消失。
  4. 高温相（400 K）并非传统体材料顺电相的完全无序：空间关联角 θij(r) 在 r<2 nm 范围内保持 75°–90°（低于完全随机的 90°），证实存在短程铁性序，即极性纳米微区。
  ...
  8. 350 K 下施加 −0.2% 双轴压缩应变可增强波纹并诱导出清晰条纹状铁电畴，而拉伸应变抑制波纹，证明"波纹工程"可通过应变/基底工程实现。
  9. 50 K 沿 x 拉伸时，无波纹 NPT 模型应力-应变曲线出现多次突发应力降（ε=4%、9–11%、16%），对应初级/次级孪晶畴的自催化级联形核生长；有波纹 NVT 模型曲线平滑，次级孪晶形核应变从 9.7% 推迟到 12.9%。
  10. 应力降幅度 PDF 是动力学判据：无波纹时服从截断幂律分布 P(A)~A^(−δ)（δ=1.3），标志高度协同雪崩动力学；有波纹时服从高斯分布，标志波纹局域应力驱动的独立随机翻转；NVT/NPT 系综本身不影响该统计（图 S12）。
