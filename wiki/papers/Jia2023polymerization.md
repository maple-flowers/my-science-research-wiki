...
"领域基础知识": >-
  飞秒激光双光子聚合是一种利用超快激光脉冲在光敏材料内部引发双光子吸收，从而实现亚微米级分辨率的三维微纳加工技术。自适应光学通过空间光调制器等可编程器件动态调控光场波前，以生成任意结构光束或补偿像差。光学像差是光学系统中偏离理想成像条件的各种误差，会导致焦点模糊、光场畸变。高阶贝塞尔光束是一种具有环形强度分布和轨道角动量的无衍射光束，对相位畸变极为敏感。
"研究背景": >-
...
"研究意义": >-
  1. 技术层面：提供了一种极简、低成本、无需额外波前传感器和复杂算法的原位像差校正方案，极大地降低了自适应光学激光加工系统的操作门槛。 2. 应用层面：显著提升了基于高阶贝塞尔光束的双光子聚合加工质量，实现了高均匀性、高圆度微管阵列的快速制造，为高质量微纳加工提供了更普适的工具。
"研究结论": >-
...
"未来研究方向提及": >-
  1. 将波前校正范围扩展到整个光学系统，包括物镜和样品界面。 2. 结合实时监测技术（如光学相干断层扫描），实现加工过程中的动态像差补偿。 3. 将该技术拓展至复振幅调制，以实现更精细的结构特征校正。
"未来研究方向思考": >-
...
- **元数据**：Erse Jia、Chen Xie、Na Xiao、Francois Courvoisier、Minglie Hu，2023，Chinese Optics Letters，21(7): 071203，DOI [10.3788/COL202321.071203](https://doi.org/10.3788/COL202321.071203)
- **一句话**：用单个纯相位 SLM 将多通道干涉波前传感原位集成到飞秒 2PP 系统中，无需泽尼克/Gerchberg–Saxton 等复杂算法即可补偿超过 4π 的系统像差，使高阶贝塞尔光束（m=6）恢复近理想形态，并快速制造出高圆度、壁厚均匀、无倒塌的 SU-8 微管阵列。

- **现有wiki双链**：
  - 年度 [[../write/2023]]
  - 项目 [[../projects/project-1-two-photon]]
  - 相关论文 [[../../raw/note/Jia2023polymerization]]
  - （wiki 中暂无光学/自适应光学/双光子聚合相关概念或实体条目可链）

- **新概念/实体建议**：
  - `concepts/two-photon-polymerization.md` — 双光子聚合（2PP）：飞秒激光在光刻胶内通过双光子吸收非线性触发局部固化，实现亚微米体素三维直写。
  - `concepts/bessel-beams.md` — 贝塞尔光束/高阶贝塞尔光束：无衍射、自愈合、带轨道角动量的环形结构光，适合单次曝光加工微管。
  - `concepts/adaptive-optics.md` — 自适应光学：用 SLM 等可编程器件动态整形/补偿波前。
  - `concepts/aberration-correction.md` — 激光加工中的光学像差校正（与泽尼克、GS 算法、波前传感并列）。
  - `concepts/wavefront-sensing.md` — 波前传感方法集合（Shack–Hartmann、多通道干涉等）。
  - `concepts/orbital-angular-momentum.md` — 光的轨道角动量（OAM）、涡旋相位 mθ。
  - `concepts/diffraction-free-beam.md` — 无衍射光束及其无衍射区/主瓣概念。
  - `entities/spatial-light-modulator.md` — 纯相位液晶空间光调制器（SLM），本文用 Holoeye PLUTO（1920×1080）。
  - `entities/SU-8.md` — MicroChem SU-8 2075 负性环氧光刻胶（本文 SU-8:环戊酮=5:1，1500 r/min 旋涂）。
  - `entities/BBO-crystal.md` — β-BBO 倍频晶体，将 1038 nm 飞秒脉冲倍频为 518/519 nm 二次谐波用于 2PP。
  - `figures/beam-intensity-profiles.md` — 结构光纵向/横向强度分布图类型。
  - `figures/sem-micrographs.md` — SEM 微结构照片图类型。

- **关键图表**：笔记未附图片（raw/figures/Jia2023polymerization/ 下仅有 manifest.json，figures 列表为空）。论文图 1–6 的内容已在笔记正文（双语转写与 AI 解读）中详述：图1 波前传感原理及校正前后傅里叶谱；图2 实验装置；图3 m=6 高阶贝塞尔光束校正前后纵/横向光强与主瓣尺寸演化；图4 微管 SEM（校正前 a–c、校正后 d–f，比例尺 20 μm）；图5 内外环圆形度；图6 m=4/6/8 贝塞尔光束加工的微管（外径 17.6/20.6/26.06 μm）。

- **项目连接**：
  - **project-1（双光固化和双光发光）—— strong（强相关）**：本文虽然在 Zotero 中被归入"01_双光子聚合引发剂"分类，但按内容参考价值判定，它是一篇直接的 2PP 加工光学/工艺论文。对项目一的可复用价值包括：(1) 给出了一套完整的 2PP 加工系统搭建参数（1038 nm→BBO 倍频 518 nm、90 fs、1 MHz、SLM Holoeye PLUTO、Olympus 20×/NA 0.4 物镜、SU-8 2075 工艺参数：1500 r/min 旋涂、65 °C/5 min + 95 °C/10 min 前烘、1.5 mW（1.5 nJ/脉冲）曝光、单管 <0.8 s）；(2) 系统讨论了结构光（贝塞尔/涡旋/自聚焦/axilens）单次曝光替代逐点扫描以提高效率的路线；(3) 提供了一种低门槛的像差原位校正方法，可直接用于项目一中任何基于 SLM 的双光固化装置的装调；(4) 阐明了像差导致能量沉积不均→聚合收缩应力不均→微结构倒塌的机制，对评估固化均匀性有参考意义。它不涉及双光子引发剂分子设计或发光机理，因此不是 core，但作为加工平台与质量控制方法学是 strong。
  - project-2/3/4/5/6/7：无直接连接（无 Mn 多铁、机械发光 NN、TTF 分子计算、SnTe 铁电、湿度传感、CDW 相关内容）。

- **组织与用词**：论文按"问题（自适应光学 2PP 中的像差）→ 原理（多通道干涉波前传感，公式 I∝|Ui|²+|Ur|²+2|Ui||Ur|cos(φi−φr)，干涉最强即同相位）→ 装置（含飞秒源、BBO 倍频、SLM、望远镜、CCD）→ 光束校正（傅里叶谱与纵/横光强对比模拟）→ 2PP 加工（SU-8 微管 SEM + 圆形度 C=4πA/P² 量化）→ 不同拓扑荷普适性（m=4/6/8）→ 结论与展望"递进。值得在 wiki 中复用的术语：
...
- **可写入wiki的要点**：
  ...
  10. 展望与局限：当前仅校正 SLM 到物镜前的像差（受空间所限未把物镜及样品界面纳入，可在物镜后加透镜监测其焦平面信号扩展到全系统）；加工前静态校正，打印过程无实时反馈，未来可结合 OCT 等实时监测形成闭环，补偿光刻胶收缩、热效应等动态畸变；可扩展到复振幅调制实现更精细特征；可推广到飞秒激光诱导周期性表面结构（LIPSS）等其他需要良好波前的激光加工场景。
