...
## 📊 关键图表
笔记正文描述了图1（TTFClO₄/PMMA薄膜真空/低温/空气暴露后的吸收光谱，标出300、800、1400 nm三条带）、图2（固定参数与释放Z₀两种方式下的Z扫描拟合曲线）、图3（两种拟合得到的CT态寿命与125 fs脉宽对比），但 `raw/figures/Scremin2018nonlinear/` 下仅含 manifest.json，无实际图片文件，**笔记未附图片**。
## 🔬 项目连接
