...
## ✏️ 可写入 Wiki 的要点
  ...
  - NiI₂ 是首个实验证实的单层 II 型 vdW 多铁体（Song et al., Nature 602, 601, 2022）：MBE 在 HOPG 上生长（基底≈100 °C，UHV≈1×10⁻¹³ bar，碘背景压≈9×10⁻¹¹ bar），T_N1=78 K 反铁磁、T_N2=59.5 K 进入螺旋磁-多铁相；铁电源于螺旋磁序 q 加强 SOC，满足 P ∝ e×q（P、q、螺旋轴 e 三垂直）。
  - NiI₂ 的多铁相在太赫兹谱中呈现 34 cm⁻¹ 和 37 cm⁻¹ 两个电磁振子模式（电偶极活性），是动态磁电耦合的指纹；圆偏振拉曼可直接探测其磁手性基态；角分辨线二色性证实单层旋转对称由 C3z 降为 C2。
  - Cr₂S₃（Song et al., Nat. Commun. 15, 721, 2024）通过 CVD 界面调制策略在工业兼容 c 面蓝宝石上生长一英寸晶圆级、单一取向的单胞（1.8 nm 厚）非层状薄膜：界面相互作用诱导自插层 Cr 原子层内滑移、破坏空间反演，产生室温铁电与超高剩余极化；铁磁 T_C=200 K（约块体两倍），是迄今最大最薄多铁材料之一。
  ...
  - p 型掺杂 SnSe（Du et al., Nat. Commun. 13, 6130, 2022）由 PVD 在云母上生长（反应器压力~10¹–10² Pa），表现 337 K 亚铁磁性与室温铁电共存，伴局域相分离 SnSe 微畴和界面电荷转移，呈简并半导体/金属特征。
  ...
  - 器件应用：MERAM 利用 ME 耦合以电场写入磁化（P↑↓控制 M↑↓），多铁隧道结可实现四态阻变存储；电场控磁相比电流控磁能耗降低 2–3 个数量级。磁电传感器在 10²–10³ Hz 低频灵敏度可达 pT–fT/√Hz，室温无源自供电，可替代 SQUID 用于脑磁图等生物磁探测。微波/RF 器件通过应变介导 ME 耦合电控铁磁共振频率（FMR），YIG/压电异质结是低损耗代表。
  - 未来五大挑战：①发现室温强耦合二维多铁材料；②实现晶圆级、层数精确可控、CMOS 兼容的制备与空气敏感材料封装；③将铁电/磁电开关电压降至 100 mV 以下；④探索三铁性（FE+FM+铁谷/铁弹，如 WRuCl₆、Nb₃X₈、双层 T′-VTe₂、GdI₂ 谷极化 155.5 meV）实现多态存储；⑤建立多铁性与量子物理（拓扑、量子纠缠、超快动力学）的联系。
