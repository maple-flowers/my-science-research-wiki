---
citekey: chenHafniumBasedFerroelectricPostMoore2026
title: "Hafnium-Based Ferroelectric Post-Moore Electronics: Device Physics, Integration Architectures, and Neuromorphic System Implementation"
authors: [Xiangwei Chen, Zheng Wang, Jialin Meng, Tianyu Wang]
year: 2026
journal: "Nano-Micro Letters"
doi: "10.1007/s40820-026-02158-z"
url: "https://doi.org/10.1007/s40820-026-02158-z"
paper_type: review
status: ingested
year_read: 2026
original_note: "[[../../raw/note/chenHafniumBasedFerroelectricPostMoore2026]]"
projects: [project-5]
concepts: [ferroelectricity, polarization-switching, ferroelectric-tunnel-junction, neuromorphic-computing, in-memory-computing, synaptic-plasticity, doping-engineering, strain-engineering, oxygen-vacancy, phase-transition, domain-wall-motion, cmos-compatibility, multiferroicity, 2d-materials, density-functional-theory, wake-up-effect, fatigue-effect, schottky-barrier, quantum-tunneling, soft-mode-phonon]
entities: [HfO2, HZO, FeFET, FTJ, FeRAM, Fe-Diode, FeCAP, MoS2, MXenes, PZT, BaTiO3, IGZO, TiN, ALD, domain-wall]
methods: [dft, high-throughput-dft, phonon-calculation, tem-stem-haadf, ald, pe-ald, rapid-thermal-annealing, device-p-e-loop, device-i-v, device-c-v, pulse-switching, kai-model, nls-model, molecular-dynamics]
materials: [HfO2, HZO-Hf0.5Zr0.5O2, Si-doped-HfO2, La-doped-HfO2, Al-doped-HfO2, Y-doped-HfO2, Ce-doped-HfO2, MoS2, MXene, IGZO, TiN]
figures: [crystal-structures, electronic-devices, device-arrays, physical-mechanisms, performance-metrics, neural-networks, image-processing, logic-operations, future-prospects]
"领域基础知识": >-
  铁电材料、非易失性存储器、神经形态计算、互补金属氧化物半导体（CMOS）工艺、后摩尔时代电子学。
"研究背景": >-
  传统冯·诺依曼架构存在"存储墙"瓶颈，导致高能耗和低吞吐量；神经形态计算作为新兴范式，急需兼容CMOS工艺、可高密度集成的纳米突触器件。
"作者的问题意识": >-
  如何系统性地梳理并阐明铪基铁电材料（Hf-FEs）从材料物理、器件结构到神经形态系统集成的完整技术链条，以揭示其在后摩尔时代非易失性存储和新型计算中的巨大潜力与关键挑战。
"主要研究对象": >-
  掺杂氧化铪（HfO₂）基铁电材料及其构成的铁电场效应晶体管（FeFET）、铁电隧道结（FTJ）、铁电随机存取存储器（FeRAM）和铁电二极管（Fe-Diode）等器件。
"主要研究方法": >-
  本文为综述，其研究方法为系统性地回顾、归纳和评述已发表文献，总结Hf-FEs在材料掺杂、器件物理、性能表征、突触模拟和系统集成等方面的研究进展。
"研究意义": >-
  为Hf-FEs领域提供了一个从基础到应用的全面视角，明确了其作为颠覆性技术在后摩尔时代推动高密度存储、存内计算和神经形态计算发展的核心地位和实现路径，为研究者提供了宝贵的结构化指南。
"研究结论": >-
  Hf-FEs凭借其卓越的CMOS兼容性和微缩能力，克服了传统铁电材料的缺点，在非易失性存储、突触可塑性模拟、图像处理和逻辑运算中展现出巨大潜力，是实现后摩尔时代高性能、低功耗计算硬件的关键使能技术之一，但仍面临亚稳相稳定、可靠性及3D集成等挑战。
"对领域的贡献": >-
  构建了"材料-器件-物理-应用-集成"的多层次知识框架，系统地连接了Hf-FEs的微观特性和宏观系统功能，并清晰地指出了该领域当前面临的核心挑战和未来发展方向。
"未来研究方向提及": >-
  探索铁电性的原子级起源；发展低于400°C的CMOS后端工艺兼容的低温制备技术；实现Hf-FEs的三维高密度集成；通过界面和共掺杂工程解决器件可靠性问题；探索多功能融合（传感-存储-计算）和跨层次协同设计。
"未来研究方向思考": >-
  开发系统级性能退化模型以量化器件差异性的影响；建立统一理论框架以指导理性材料设计而非经验性试错；深入探索Hf-FEs与2D材料等异质集成形成的新器件物理；建立包含外围电路开销的公正、全面的系统级能效评估标准。
tags:
  - paper
  - type/review
  - year/2026
  - project/project-5
  - relevance/project-5/medium
  - concept/ferroelectricity
  - concept/polarization-switching
  - concept/ferroelectric-tunnel-junction
  - concept/neuromorphic-computing
  - concept/in-memory-computing
  - concept/synaptic-plasticity
  - concept/doping-engineering
  - concept/strain-engineering
  - concept/oxygen-vacancy
  - concept/phase-transition
  - concept/domain-wall-motion
  - concept/cmos-compatibility
  - concept/multiferroicity
  - concept/2d-materials
  - concept/density-functional-theory
  - concept/wake-up-effect
  - concept/fatigue-effect
  - concept/schottky-barrier
  - concept/quantum-tunneling
  - concept/soft-mode-phonon
  - entity/HfO2
  - entity/HZO
  - entity/FeFET
  - entity/FTJ
  - entity/FeRAM
  - entity/Fe-Diode
  - entity/FeCAP
  - entity/MoS2
  - entity/MXenes
  - entity/PZT
  - entity/BaTiO3
  - entity/IGZO
  - entity/TiN
  - entity/ALD
  - entity/domain-wall
  - method/dft
  - method/high-throughput-dft
  - method/phonon-calculation
  - method/tem-stem-haadf
  - method/ald
  - method/pe-ald
  - method/rapid-thermal-annealing
  - method/device-p-e-loop
  - method/device-i-v
  - method/device-c-v
  - method/pulse-switching
  - method/kai-model
  - method/nls-model
  - method/molecular-dynamics
  - material/HfO2
  - material/HZO-Hf0.5Zr0.5O2
  - material/Si-doped-HfO2
  - material/La-doped-HfO2
  - material/Al-doped-HfO2
  - material/Y-doped-HfO2
  - material/Ce-doped-HfO2
  - material/MoS2
  - material/MXene
  - material/IGZO
  - material/TiN
  - topic/ferroelectricity
  - topic/neuromorphic-computing
  - topic/in-memory-computing
  - topic/nonvolatile-memory
  - topic/post-moore-electronics
  - topic/2d-materials
  - topic/multiferroics
---

## chenHafniumBasedFerroelectricPostMoore2026 — 铪基铁电后摩尔电子学：器件物理、集成结构和神经形态系统实现

- **元数据**：Xiangwei Chen, Zheng Wang, Jialin Meng, Tianyu Wang（山东大学），2026，Nano-Micro Letters，18:326，DOI: 10.1007/s40820-026-02158-z
- **一句话**：本文系统综述了与CMOS工艺兼容的铪基铁电体（Hf-FEs）从掺杂材料工程、四类器件结构（FeFET/FTJ/FeRAM/Fe-Diode）、极化翻转微观机制到神经形态存算一体系统实现的完整技术链条，并批判性地指出亚稳相稳定、BEOL热预算、可靠性与3D集成等核心挑战。
- **现有wiki双链**：
  - 概念 [[../concepts/ferroelectric-tunnel-junction]]
  - 概念 [[../concepts/polarization-switching]]
  - 概念 [[../concepts/strain-engineering]]
  - 概念 [[../concepts/density-functional-theory]]
  - 概念 [[../concepts/2D-materials]]
  - 概念 [[../concepts/multiferroicity]]
  - 实体 [[../entities/domain-wall]]
  - 实体 [[../entities/MXenes]]
  - 实体 [[../entities/TMDs]]（MoS₂）
  - 图表 [[../figures/crystal-structures]]
  - 图表 [[../figures/electronic-devices]]
  - 图表 [[../figures/heterostructures-stacking]]
  - 年度 [[../write/2026]]
  - 项目 [[../projects/project-5-snte-ferroelectric-sim]]
  - 相关论文 [[../../raw/note/chenHafniumBasedFerroelectricPostMoore2026]]
- **新概念/实体建议**：
  - `concepts/neuromorphic-computing.md` — 神经形态计算：模拟生物神经系统并行、事件驱动工作模式的计算范式，Hf-FEs通过模拟突触可塑性实现硬件加速。
  - `concepts/in-memory-computing.md` — 存内计算/存算一体：在存储单元内直接完成向量-矩阵乘法（VMM），打破冯·诺依曼存储墙。
  - `concepts/synaptic-plasticity.md` — 突触可塑性：LTP/LTD、STDP、PPF/PPD、SADP/SWDP/SRDP等学习规则的硬件模拟。
  - `concepts/oxygen-vacancy.md` — 氧空位（V_O²⁺）：HfO₂中既是铁电相稳定剂（>2%浓度使o相热力学稳定）又是疲劳/印记效应来源的双重角色缺陷。
  - `concepts/domain-wall-motion.md` — 畴壁运动：(2;0)型低能垒（73 meV）路径主导HfO₂极化翻转，区别于传统(1;1)型1.36 eV高能垒。
  - `concepts/cmos-compatibility.md` — CMOS兼容性与BEOL热预算：HfO₂作为高k栅介质的天然工艺兼容性，但结晶温度（>450 °C）与BEOL限制（<400 °C）存在冲突。
  - `concepts/wake-up-fatigue.md` — 唤醒效应与疲劳：铁电薄膜初始循环中Pr增大（氧空位脱钉）与长期循环后Pr衰减（畴壁钉扎）。
  - `concepts/soft-mode-phonon.md` — 软模声子/平带声子机制：Lee等通过第一性原理发现极性声子平带诱导无标度铁电序，是HfO₂铁电性本征起源理论之一。
  - `entities/HfO2.md` — 氧化铪：萤石结构铁电母体，铁电相为亚稳态正交相Pca2₁。
  - `entities/HZO.md` — Hf₀.₅Zr₀.₅O₂：最主流的Zr掺杂HfO₂铁电体系，400–600 °C宽结晶窗口。
  - `entities/FeFET.md` — 铁电场效应晶体管：三端MFS/MFMIS/MFIS结构，通过极化调制阈值电压。
  - `entities/FeRAM.md` — 铁电随机存取存储器：1T-1C FeCAP结构，耐久性可达10¹²–10¹³次。
  - `entities/Fe-Diode.md` — 铁电二极管：基于肖特基势垒调制的两端自选择器件，适合3D交叉阵列。
- **关键图表**：
  - 图1：Hf-FEs材料、器件结构与应用总览
  ![Hf-FEs材料器件应用总览](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_1_3VD9GL58.png)
  - 图2：2011–2025年Hf-FEs关键进展时间线
  ![Hf-FEs发展时间线](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_2_FL9DP299.png)
  - 图3：掺杂HfO₂晶体结构、STEM/TEM图像与器件制造工艺流程
  ![掺杂工程与制造工艺](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_3_XHE7JUE9.png)
  - 图4：FeFET/FTJ/FeRAM/Fe-Diode四类器件结构与阵列
  ![四类器件结构](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_4_L3JZI8BN.png)
  - 图5：物理机制——声子平带、相变、DFT掺杂筛选、应变、氧空位、畴壁运动与氧离子迁移路径
  ![物理机制图解](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_5_CWFJTDMK.png)
  - 图6：P-E回线、I-V/C-V曲线、漏电流、开关速度、热稳定性、耐久性、均匀性与保持性
  ![性能表征图集](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_6_N4SVXSM2.png)
  - 图7：存储应用——TCAM/CECAM阵列与2D/3D FeNAND
  ![存储应用](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_7_5ZNA8QXN.png)
  - 图8：突触可塑性——EPSC/PPF/LTP-LTD/SADP/SWDP/SRDP/STDP/学习-遗忘-再学习
  ![突触可塑性](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_8_49WAIQHG.png)
  - 图9：神经网络——SLP/CNN/SNN/VGG-11架构、MNIST识别与芯片集成
  ![神经网络系统](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_9_6UW5JTH3.png)
  - 图10：图像处理——ANN/CNN、储备池计算、图像加密、贝叶斯医学诊断、边缘检测
  ![图像处理应用](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_10_LT3D7PBR.png)
  - 图11：存内逻辑运算——AND/OR/NOR/NAND可重构布尔逻辑
  ![逻辑运算](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_11_MTJAQVFZ.png)
  - 图12：未来展望与挑战思维导图
  ![挑战与展望](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/fig_12_CFHFZM9L.png)
  - 表1：不同掺杂HfO₂铁电薄膜性能对比（掺杂剂/浓度/厚度/堆叠/2Pr/Ec/热稳定性）
  ![掺杂HfO2性能对比表](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/tab_1_FSTFNUCW.png)
  - 表2：不同HfO₂铁电器件性能交叉对比（结构/开关速度/保持/耐久性/功耗）
  ![器件性能对比表](../../raw/figures/chenHafniumBasedFerroelectricPostMoore2026/tab_2_UQKGTEBW.png)
- **项目连接**：
  - **project-5 SnTe铁电模拟（medium）**：本文虽以HfO₂薄膜器件为核心，不直接涉及SnTe，但在铁电物理与计算方法层面对project-5有明确参考价值：
    1. **极化翻转微观机制**：综述详细总结了畴壁运动（(2;0)型路径能垒仅73 meV，对比(1;1)型1.36 eV）与氧离子协同迁移（层内/层间路径）两种翻转机制，这一"低能垒畴壁路径"图像可类比SnTe中铁电畴翻转的能垒分析。
    2. **应变工程稳定铁电相**：Fan等证明>3%双轴拉伸应变使Pca2₁相比反极性Pbca相更稳定；泊松效应通过面内拉伸驱动t→o相变。这为SnTe应变调控铁电性提供了可类比的热力学框架。
    3. **DFT方法学参考**：Rohit Batra等用高通量第一性原理DFT筛选近40种掺杂剂对HfO₂相稳定性的影响，He等用DFT证明V_O²⁺浓度>2%时o相热力学稳定——这些掺杂/缺陷筛选的计算流程可迁移至SnTe的缺陷与应变计算。
    4. **软模声子图像**：Lee等通过第一性原理发现极性声子平带诱导无标度铁电序，这一声子软化机制与SnTe铁电失稳的软模图像相通。
    5. **FTJ器件物理**：铁电隧道结的隧穿电致电阻（TER）效应、极化调制势垒形状的概念，对理解SnTe作为铁电势垒层的器件物理有类比价值。
    局限：HfO₂为萤石结构氧化物、实验器件导向，SnTe为IV-VI族岩盐结构、DFT基础研究导向，材料体系与研究尺度差异较大，故评为medium而非strong。
  - **project-2 Mn多铁（weak）**：仅在展望中一句提及"基于HfO₂的多铁性材料有望应用于自旋轨道转矩器件"，无实质性Mn或多铁机制讨论，参考价值有限。
  - project-1双光子、project-3机械发光NN、project-4 TTF分子计算、project-6湿度传感器、project-7 CDW：无直接项目连接。
- **组织与用词**：文章采用"背景问题→材料基础→器件结构→物理机制→性能指标→存储应用→神经形态计算→挑战展望"的总-分-总递进结构，逻辑链条是"冯·诺依曼瓶颈→Hf-FEs材料优势（CMOS兼容/微缩）→掺杂工程稳定o相→四类器件各有侧重→微观翻转机制→突触可塑性模拟→交叉阵列VMM→系统级集成挑战"。每个器件小节均按"结构原理→代表性性能数据→现存挑战"三段式展开。值得复用的关键词/术语：
  - 亚稳态极性正交相（metastable polar orthorhombic phase, Pca2₁）
  - 无标度铁电性（scale-free ferroelectricity）/ 平带声子（flat phonon bands）
  - 畴壁运动（domain wall motion）/ (2;0)-type vs (1;1)-type DW
  - 成核限制开关模型（nucleation-limited switching, NLS）/ KAI模型
  - 隧穿电致电阻比（tunneling electroresistance ratio, TER）
  - 存内计算/存内逻辑（in-memory computing / logic-in-memory, LiM）
  - 向量-矩阵乘法（vector-matrix multiplication, VMM）
  - 唤醒/疲劳/印记效应（wake-up / fatigue / imprint effect）
  - BEOL热预算（back-end-of-line thermal budget, <400 °C）
- **可写入wiki的要点**：
  1. HfO₂铁电性源于亚稳态极性正交相（o-phase, Pca2₁），而非稳定的单斜相（m-phase, P2₁/c）；温度序列为m→t（P4₂/nmc, >1973 K）→c（Fm3̄m, >2773 K）。2011年Böscke等首次在Si:HfO₂中发现铁电性。
  2. 掺杂工程是稳定o相的核心手段：Zr掺杂（HZO）工艺窗口最宽（400–600 °C结晶，50%浓度）；La掺杂稳定效应最强，2Pr可达55 μC cm⁻²（800 °C退火），并抑制氧空位；Al掺杂通过压应变稳定超薄膜铁电相；La-Al共掺杂协同调控氧空位与应变。
  3. 四类器件性能对比：FeFET（三端，MFS/MFMIS，开关比3.5×10⁶，存储窗口2 V）；FTJ（两端，500 ps开关，0.12 fJ/bit写入，>10⁷次循环）；FeRAM（1T-1C FeCAP，1.2 V低压，>10¹²–10¹³次循环，10年保持）；Fe-Diode（肖特基势垒调制，自选择抑制潜行电流，800 ps开关，16层3D堆叠，面积效率0.06 F²/state）。
  4. 铁电性起源三大理论：声子带驱动机制（软模→晶格失稳，Lee等平带声子诱导无标度铁电序）；缺陷介导机制（V_O²⁺>2%使o相热力学稳定于m相，解释唤醒/疲劳）；界面/尺寸约束机制（纳米尺度表面/界面能主导相稳定性）。当前共识为多尺度多物理场协同。
  5. 极化翻转微观路径：(2;0)型畴壁路径能垒仅73 meV（几何上类似t相），远低于(1;1)型1.36 eV；0.4%拉伸应变可消除过渡势垒。氧离子迁移分"层内迁移"（Op原位反转偶极）与"层间迁移"（Onp跨极性/非极性边界交换身份）两条路径。
  6. 宏观开关动力学：多晶HfO₂薄膜由成核过程主导（NLS模型），而非经典KAI模型的成核后自由横向扩展；Cristóbal Alessandri等提取HZO的最小开关时间τ∞≈236 ns、激活场Ea≈2.4 MV cm⁻¹、局域场增强因子符合广义贝塔分布。
  7. 应变调控定量结果：Fan等证明>3%双轴拉伸应变使Pca2₁相比反极性Pbca相更稳定；面内拉伸通过泊松效应引起面外收缩，驱动t→o相变。
  8. DFT掺杂筛选：Rohit Batra等用高通量DFT研究近40种掺杂剂，发现离子半径大、电负性低的掺杂剂（镧系、Ca/Sr/Ba/Y等）最有效稳定P-O1相。
  9. 神经形态实现：Hf-FEs交叉阵列通过欧姆定律+基尔霍夫定律在模拟域物理完成VMM；SLP在MNIST达90%准确率，SNN 50个epoch达~98%且抗噪声，VGG-11用于CNN评估；AFE HZO电容突触能耗245 fJ/spike，保持>10年，>10⁹次循环；1T-1R忆阻芯片能效119.7 TOPS/W，10 ns读出。
  10. 核心挑战：o相亚稳性可靠稳定；氧空位双重角色的精确纳米尺度控制；存储窗口-耐久性-低电压三角权衡；BEOL热预算冲突（结晶>450 °C vs BEOL <400 °C，PE-ALD可在300 °C实现2Pr≈20–40 μC cm⁻²）；3D集成的高深宽比保形沉积；器件间/循环间变异性的系统级容错。
