...
"领域基础知识": >-
  光纤湿度传感器利用光纤中光的倏逝场与环境水分子相互作用，导致光衰减变化。二维材料（如石墨烯、过渡金属硫族化合物）因高比表面积和可调电子性质，被用作敏感涂层以增强相互作用。相对衰减差分（RDA）定义为|ΔA|/A₀，是衡量传感器响应的核心指标。材料表面官能团决定其半导体类型（n型或p型），从而影响水分子吸附后的载流子浓度和折射率变化方向。
"研究背景": >-
...
- **元数据**：Owji, Mokhtari, Ostovari, Darazereshki, Shakiba，2021，Scientific Reports 11:1771，DOI 10.1038/s41598-020-79563-w
- **一句话**：首次用 HF 化学蚀刻减薄单模光纤并分别涂覆 G/GO、MoSe₂、MoS₂ 三种二维材料，在 1310/1550 nm 通信波长下系统比较其湿度传感性能，证明 G/GO 涂层在 20–90% RH 全量程具有高且单调的 RDA 响应，并用表面官能团–半导体类型（n/p）–Penn 模型折射率框架统一解释了三种材料的行为差异。
...
- **组织与用词**：
  - 文章按"引言（动机与文献缺口）→ 材料与方法（合成三小节 + 器件制备）→ 结果（性能曲线 + 机理解释）→ 结论"的经典材料学实验范式组织。逻辑链是：化学蚀刻制备 ESMF（几何改造）→ 三种二维材料浸涂（化学修饰）→ OLTS 测 RDA-RH（性能对比）→ FTIR 判定 n/p 型 + Penn 模型（机理统一）→ 按工况给出材料选择建议。论证以"性能差异—表面官能团—半导体类型—折射率变化方向"的因果链贯穿全文。
  - 值得复用的关键词/术语：
