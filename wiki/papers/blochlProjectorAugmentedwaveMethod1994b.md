...
- **元数据**：P. E. Blöchl，1994，Physical Review B 50(24), 17953–17979，DOI: 10.1103/PhysRevB.50.17953
- **一句话**：本文提出投影增强波（PAW）方法，通过一个由全电子分波、赝分波和投影函数定义的线性变换，将全电子 LAPW 方法的精度与平面波赝势方法的效率统一起来，并首次实现基于完整波函数的能量守恒第一性原理分子动力学。
...
- **新概念/实体建议**：
  - `paw-method`：投影增强波法，通过线性变换 T 联系全电子波函数与赝波函数，是 VASP 等主流代码的核心算法。
  - `projector-functions`：投影函数，局域在增强区域内、满足双正交关系 ⟨p̃_i|φ̃_j⟩=δ_ij，从赝波函数提取分波系数，是 PAW 区别于 LAPW 球面匹配的核心创新。
  - `augmentation-region`：增强区域（augmentation region），围绕原子的球形区域，对应 LAPW 的 muffin-tin 球或赝势的芯区，AE 与 PS 波函数仅在此区域内不同。
  - `compensation-charge-density`：补偿电荷密度 n̂，用广义高斯函数构造，使单中心修正项具有零多极矩，把静电相互作用转移到平面波部分，保证计算高效精确。
  - `additive-augmentation`：加法缀加原理，AE 与 PS 分波以完全相同方式截断，使高阶分波由平面波尾部表示、基组完备性不受截断影响，并促成误差抵消。
  - `frozen-core-approximation`：冻结芯近似，芯态从孤立原子导入、不随环境变分弛豫；PAW 中仍允许芯态间幺正混合。
  - `overlap-operator`：重叠算符 Õ，放松范数守恒后平面波不再正交，PAW 需在非平凡重叠算符下正交化波函数，这是与范数守恒赝势的根本区别。
...
- **可写入wiki的要点**：
  ...
  - 力分三部分：F_R = F_R⁽¹⁾+F_R⁽²⁾+F_R⁽³⁾，分别对应平滑部分刚性位移、单中心密度形状变化（Pulay 力）、波函数正交性变化；力与总能量必须一致才能保证 MD 能量守恒。
