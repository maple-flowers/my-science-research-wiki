...
"未来研究方向思考": >-
  可进一步研究缺陷（如氧空位）或掺杂对涡旋-反涡旋成核和移动的钉扎作用，以探索实现畴结构定点操控的可能性。此外，可尝试将这种二维涡旋畴壁的动力学研究扩展到三维体系，考虑涡旋线沿另一方向的弯曲和缠绕等复杂行为。
tags:
  - paper
  - type/theory
  - year/2023
...
- **元数据**：Fernando Gómez-Ortiz, Hugo Aramberri, Juan M. López, Pablo García-Fernández, Jorge Íñiguez, Javier Junquera et al.，2023 年，Physical Review B 107, 174102，DOI [10.1103/PhysRevB.107.174102](https://doi.org/10.1103/PhysRevB.107.174102)
- **一句话**：用第二性原理模拟首次在具有反向旋转极化涡旋畴壁的 (PbTiO₃)ₙ/(SrTiO₃)ₙ 超晶格中验证 Kittel 定律 ω²∝d，并揭示亚稳态通过界面涡旋-反涡旋对成核、拉长、合并与湮灭（缺陷复合）动态形成新畴的微观机制。
- **现有wiki双链**：
  - 概念 [[../concepts/multiferroicity]]、[[../concepts/topological-defects]]、[[../concepts/strain-engineering]]、[[../concepts/ferroelasticity]]、[[../concepts/polarization-switching]]
...
- **新概念/实体建议**：
  - `kittel-law.md`（Kittel/朗道-基特尔定律）：畴宽 ω 与膜厚 d 的平方根标度 ω²/δ=Ad，畴壁能与静电/退极化能平衡的经典结果，应在 wiki 中作为铁性畴物理的基础条目。
  - `polar-vortices.md`（极性涡旋）：PbTiO₃/SrTiO₃ 超晶格中由顺时针/逆时针涡旋序列构成的非 Ising 畴壁，轴向（y）极化分量、涡旋中心偏移等关键图像。
  - `vortex-antivortex-pair.md`（涡旋-反涡旋对）：拓扑荷 +1/−1 的偶极纹理缺陷，受庞加莱-霍普夫总涡度为零约束，其复合/湮灭驱动畴形成。
  - `depolarization-field.md`（退极化场）：表面束缚电荷产生的反向电场，是 Kittel 平衡中静电能项的物理来源。
  - `second-principles.md`（第二性原理方法）：基于 DFT 参数化的有效模型势（SCALE-UP），介于第一性原理与经典 MD 之间，可处理大尺度铁电畴。
  - `superlattice.md`（超晶格）：两种钙钛矿薄层周期交替的铁电/介电体系，本文核心研究对象类别。
  - `anharmonic-effects.md`（非谐效应）：谐波力常数分析预测畴宽偏窄的原因，极化倾斜/涡旋偏移降低退极化电荷。
  - `polarization-waves.md`（极化波）：高畴密度亚稳态（L<Lc）下沿 [100]pc 的波浪状极化调制，净面内极化出现。
  - 实体 `PbTiO3.md`、`SrTiO3.md`、`PbTiO3-SrTiO3-superlattice.md`：钙钛矿铁电/顺电端元及其超晶格。
  - 实体 `SCALE-UP.md`、`PHONOPY.md`：第二性原理软件包与声子/力常数计算工具。
- **关键图表**：
  ...
  - 图2：不同 n 下单胞能量随横向尺寸 L 的曲线及 ω²–n 线性拟合（Kittel 定律验证，红点能量最小化、蓝方块力常数法）
    ![图2 Kittel定律验证](../../raw/figures/gomez-ortizKittelLawDomain2023/fig_2_AAWRLAWB.png)
...
- **组织与用词**：文章采用"提出问题→验证经典定律→揭示新机制→结论"的标准 PRB 结构。方法论部分先交代 SCALE-UP 第二性原理势的拟合来源（DFT+LDA，−11.2 GPa 静水压修正立方晶格常数低估）、外延约束与超胞尺寸（退火用 L×1×2n，MD 用 L×10×2n），再把结果分为 A. Kittel 定律验证（能量最小化与力常数谱两条独立证据，系统差异归因非谐效应）和 B. 畴形成（2 畴/4 畴能量竞争定 Lc，再用 MD 拍涡旋-反涡旋复合全过程）。值得复用的关键词：
  ...
  - depolarization field / electrostatic energy — 退极化场 / 静电能
  - domain wall energy vs gradient energy — 畴壁能与梯度能
  - force-constant band / soft mode — 力常数带 / 软模
...
- **可写入wiki的要点**：
  1. Kittel 定律在 (PbTiO₃)ₙ/(SrTiO₃)ₙ（n=8–16 u.c.）的极性涡旋相中成立：畴壁能与静电能平衡给出最优畴周期 ω，其平方与 PbTiO₃ 层厚 d 线性相关，ω²/δ=Ad（δ≈1 u.c. 为畴壁厚度）。
  ...
  5. 低畴密度亚稳态（畴宽过大，L>Lc）在 T<50 K 长寿命存在；升温至 ≥90 K 或施加 ≥−0.5% 压应变可越过势垒，自发形成新畴。高畴密度亚稳态（L<Lc，极化波态）即使升温也无法弛豫到低密度态——这种不对称性源于静电能与梯度能势垒形态不同。
  6. 畴形成的微观动力学四步：界面处涡旋-反涡旋对成核→涡旋沿界面拉长并将反涡旋推向 PbTiO₃ 层中心→两个反涡旋合并为涡度−2 的高能双反涡旋（能量随涡度平方增加，符合 Kosterlitz-Thouless XY 模型）→约 50 fs 内与两个涡旋湮灭，形成向下极化新畴与新的拉长顺/逆时针涡旋对，最终展宽至 Kittel 最优尺寸。整个过程发生在数百飞秒内（n=14, L=28, T=90 K, −0.5% 应变）。
  ...
  9. 计算细节可复用：第二性原理势由 DFT(LDA) 拟合，施 −11.2 GPa 静水压修正立方晶格常数低估；外延约束 a=b=3.901 Å 模拟 SrTiO₃ 衬底；局域极化用 Born 有效电荷张量×原子位移/单元胞体积的线性近似；力常数带含 Gonze-Lee 非解析项以分裂纵/横极性带。
  10. 历史脉络：Landau-Lifshitz(1935)→Kittel(1946,1949) 铁磁畴→Mitsui-Furuichi(1953) 罗谢尔盐推广到铁电→Stephanovich 等铁电/顺电超晶格→Roitburd 外延铁弹薄膜→Lai/Bellaiche 等 PZT 与 Prosandeev/Bellaiche 等 BiFeO₃ 超薄膜第一性原理验证（薄至 3 u.c.）→本文推广到极性涡旋相，并补上动力学机制。
