---
citekey: XiaokangZhang2013calibrating
...
authors: [Xiaokang Zhang, Zhuoyong Deng, Hanlang Xu]
year: 2013
journal: "Chinese Optics Letters"
doi: "10.3788/COL201311.090604"
url: "https://doi.org/10.3788/COL201311.090604"
...
figures: [experimental-setups, optical-spectra, mathematical-models]
"领域基础知识": >-
  **相对湿度**是气体中实际水汽压与同温度下饱和水汽压的比值，具有温度依赖性。光纤湿度传感器利用**消逝场**原理，通过感湿材料（如**琼脂糖**）折射率的变化来调制光信号，具有抗电磁干扰等优点。
"研究背景": >-
  传统电子湿度传感器存在局限，而基于光功率变化的光纤湿度传感器虽成本低，但此前研究仅限单一温度，其在变温环境下的响应特性及实际应用尚属空白。
"作者的问题意识": >-
  如何使一种结构简单、成本低廉的光纤湿度传感器，在温度变化的实际环境中实现精确、可靠的**相对湿度**测量，并将其应用于复杂恶劣的工业现场？
"主要研究对象": >-
  一种基于**双层包层单模光纤**结构、涂覆**琼脂糖**凝胶的湿度传感器，及其在25-34°C、30-100% RH范围内的响应特性和校准方法。
"主要研究方法": >-
  1. 传感器制备与加速老化；2. 在**气候试验箱**中进行多温度、多湿度的系统性性能表征；3. 基于实验数据构建**查找表**校准矩阵；4. 在新拌混凝土中进行实时、原位监测验证。
"研究意义": >-
  为光纤湿度传感器在变温环境下的应用提供了首个系统性的性能分析，并提出了一个简单实用的工程化校准方法，推动了该类传感器从实验室走向实际工程应用，尤其是土木工程监测领域。
"研究结论": >-
  传感器具有低插入损耗和高灵敏度。其响应受温度和湿度耦合影响，呈现非线性特征。提出的**查找表**校准法能有效解决此问题。成功监测了混凝土硬化过程中湿度从100%降至68.9%、温度先升后降的动态过程，验证了方法的可行性。
"对领域的贡献": >-
  1. 首次揭示了**琼脂糖**涂层吸湿能力随湿度增加而减弱的现象；2. 提出了一种不依赖复杂模型的**查找表**校准法；3. 验证了光纤传感器在混凝土内部进行原位、长期监测的可行性。
"未来研究方向提及": >-
  文中未明确提及，但根据研究内容可推断为：传感器的长期稳定性和耐久性测试，以及该传感系统在其他领域的应用拓展。
"未来研究方向思考": >-
  1. 开发具有物理意义的理论模型，以增强校准方法的外推能力；2. 实现单一光纤上温度和湿度的同步解调，简化系统；3. 研究**琼脂糖**材料在混凝土等高碱性环境下的化学稳定性问题。
tags:
  - paper
  - type/experiment
  - year/2013
  ...
  - concept/optical-power-variation
  - concept/cross-sensitivity
  - concept/lookup-table-calibration
  - concept/hydrogel-swelling
  - concept/temperature-compensation
  - entity/agarose
  - entity/dcsmf
  - entity/corning-smf-28e
  - entity/dfb-ld
  - entity/pmma
  - entity/pvdf
  - entity/climatic-chamber
  - entity/optical-power-meter
  - method/fiber-optic-sensing
  - method/evanescent-field-sensing
  - method/chemical-etching
  - method/fusion-splicing
  - method/wet-dry-aging
  - method/climatic-chamber-characterization
  - method/lookup-table-calibration
  ...
  - topic/civil-engineering-monitoring
---

## XiaokangZhang2013calibrating — 光纤湿度传感器的标定及其在新拌混凝土相对湿度实时监测中的应用

- **元数据**：Xiaokang Zhang、Zhuoyong Deng、Hanlang Xu，2013，Chinese Optics Letters，10.3788/COL201311.090604
- **一句话**：制备了一种琼脂糖涂覆的双层包层单模光纤（DCSMF）湿度传感器，首次系统研究其在 25–34 °C、30–100% RH 范围内的温-湿耦合响应，提出基于查找表/校准矩阵的工程化校准方法，并成功埋入新拌混凝土实现 33 小时内部相对湿度与温度的实时原位监测。

- **现有wiki双链**：
  - 概念：本仓库现有概念条目以二维材料、多铁、铁电、CDW 为主，与本文（光纤湿度传感）无直接重合，暂不双链。
  - 实体：同上，暂不双链。
...
- **新概念/实体建议**：
  - `concepts/evanescent-field-sensing.md` — 倏逝场传感：纤芯外渗透的电磁场与外包层敏感材料相互作用，将环境物理量转化为光功率/波长变化。
  - `concepts/relative-humidity.md` — 相对湿度定义及其温度依赖性（饱和水汽压经验公式 P_S = 6.1078·exp[17.269T/(T+237.3)]）。
  - `concepts/lookup-table-calibration.md` — 查找表/校准矩阵法：当传感器输出与多变量呈非线性耦合、无法解析拟合时，用离散实验数据插值构建高维"数字地图"反演被测量。
  - `concepts/cross-sensitivity.md` — 交叉敏感（温度-湿度耦合），以及双传感器+矩阵解耦的工程思路。
  - `concepts/hydrogel-swelling.md` — 水凝胶（琼脂糖）吸湿溶胀导致折射率变化的物理机制。
  - `entities/agarose.md` — 琼脂糖：从海藻提取的多糖水凝胶，常用作光纤倏逝场湿度敏感涂层。
  - `entities/dcsmf.md` — 双层包层单模光纤（Doubly Cladding Single-Mode Fiber），外包层可被腐蚀并替换为敏感材料。
  - `entities/pmma-pvdf-blend.md` — PMMA/PVDF（49:51 质量比）共混聚合物，用作本文温度光纤传感器的外包层（但仍有湿度交叉敏感）。
...
- **组织与用词**：文章按"器件制备与老化 → 多温多湿表征 → 现象分析（温-湿耦合、非线性分段）→ 查找表校准矩阵构建 → 混凝土原位监测验证"的经典实验链条推进；当作者明确指出"无法拟合出将 RH 与传感器衰减联系起来的表达式"后，转而用工程化数据驱动方法绕开解析建模，是全文论证的枢纽。可在 wiki 叙述中复用的术语：
  - 双层包层单模光纤（DCSMF, doubly cladding single-mode fiber）
  - 倏逝场传感（evanescent field sensing）
...
- **可写入wiki的要点**：
  1. 传感器结构与工艺：裸 Corning SMF-28e 光纤总长约 100 cm，中心约 2 cm 区段的原包层被化学腐蚀部分去除（剩余部分作内包层），涂覆琼脂糖颗粒+去离子水配制的水凝胶作为外包层；传感区固定于 7×3×0.2 cm 镂空平板上，两侧放置直径 0.9 mm、长 4 cm 铜线作围挡与保护；室温干燥一天后两端熔接尾纤。
  2. 光学系统：1550 nm 稳频 DFB-LD 光源（输出功率 0.08 dBm）经光分路器均分进入湿度传感器与温度传感器，输出由两个光功率计检测、两台计算机记录；传感探头置于 Binder KBF 115 气候试验箱。
  ...
  10. 方法局限与批判点：查找表法无外推能力（仅限 25–34 °C、30–100% RH）；100% RH 参考点由滴水/浸水获得，与气相 100% RH 平衡的水合状态可能存在系统偏差；洁净空气中标定的矩阵在混凝土高碱（pH>12）、含 K⁺/Na⁺/OH⁻ 离子环境中可能因离子渗入涂层而失效；约 2 cm 传感区给出的是空间平均湿度，空间分辨率有限；温度传感器需用温度计二次校准，存在循环依赖风险。
