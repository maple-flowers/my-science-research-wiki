...
## 📊 关键图表
笔记未附图片（raw/figures/H2017fluorescence/ 下仅有 manifest.json，未抽取到实际图片文件）。文中关键图表包括：图1 P1 在 10 种溶剂中的归一化单光子发射光谱（λex=410 nm，445→641 nm）；图2 发射波数对 ET(30)、Z、δΔG≠、π*、Δf 等溶剂参数的线性拟合（r²=0.82–0.90）；图3 P1 在甘油中随温度（20–70 °C，单/双光子）和在甘油-乙醇混合溶剂中随粘度（单/双光子）变化的发射光谱，双光子谱中出现 B/A/E 三重荧光带；表1 P1 在 10 种溶剂中的吸收峰、发射峰、量子产率、双光子激发峰（790 nm）和 δmax（环己烷 5560 GM，DMF 130 GM）。
