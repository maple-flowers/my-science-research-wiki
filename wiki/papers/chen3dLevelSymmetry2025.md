...
"领域基础知识": >-
  自旋电子学(Spintronics)旨在利用电子自旋进行信息存储与处理。二维材料如过渡金属二硫化物(TMDs)和MXenes因其独特的电子和磁学性质，成为该领域的研究热点。半金属性(Half-metallicity)是一种仅对单一自旋取向电子导电的理想状态，可实现100%自旋极化电流。密度泛函理论(DFT)是计算材料电子结构的核心方法。
"研究背景": >-
...
"主要研究方法": >-
  基于密度泛函理论(DFT)的第一性原理计算，采用VASP软件包，使用GGA+U方法处理Mn 3d电子的强关联效应。通过计算态密度(DOS)、分态密度(PDOS)、投影能带(PBAND)、静电势和电子局域函数(ELF)来系统分析材料的电子结构、电子转移和成键特性。
"研究意义": >-
...
- **元数据**：Kaiyun Chen、Xue Yan、Junkai Deng、Yuan Yan、Jiabei He、Dongxiao Kan、Wangtu Huo、Le Zhang、Jefferson Zhe Liu，2025，Physical Review B 112, 085118，DOI: [10.1103/l1lf-d6hc](https://doi.org/10.1103/l1lf-d6hc)
- **一句话**：通过DFT计算揭示Mn2N MXenes的半金属性/半导体性由两侧Mn原子3d能级分离程度（而非局域配位对称性）决定，并据此用电荷掺杂和单轴应变实现两种态的可逆切换。

- **现有wiki双链**：
  - 概念 [[../concepts/density-functional-theory]]、[[../concepts/strain-engineering]]、[[../concepts/spin-orbit-coupling]]、[[../concepts/2D-materials]]、[[../concepts/electron-counting-rule]]
  - 实体 [[../entities/MXenes]]、[[../entities/TMDs]]、[[../entities/VASP]]、[[../entities/h-BN]]
  - 图表 [[../figures/crystal-structures]]、[[../figures/electronic-bands]]
  - 年度 [[../write/2025]]
  - 相关论文 [[../../raw/note/chen3dLevelSymmetry2025]]

- **新概念/实体建议**：
  - `concepts/half-metallicity.md` — 半金属性：一种自旋通道导电、另一自旋通道绝缘，可产生100%自旋极化电流
  - `concepts/janus-structure.md` — Janus结构：上下表面具有不同化学组成/官能团的不对称二维材料
  - `concepts/crystal-field-theory.md` — 晶体场理论：配位环境决定过渡金属d轨道分裂与电子占据的理论框架
  - `concepts/charge-doping.md` — 电荷掺杂：通过注入电子或空穴直接改变费米能级附近轨道占据的调控手段
  - `concepts/electronic-phase-transition.md` — 电子相变：由外场驱动的金属/半导体/半金属态之间的转变
  - `entities/Mn2N.md` — Mn2N MXene母体，两侧Mn夹一层N的反1T-TMD三明治结构
  - `entities/Mn2NOF.md` — Janus Mn2NOF，本文半导体性的代表体系
  - `entities/Mn2NO2.md` — 对称Mn2NO2，半金属性的代表体系
  - `entities/V2NOF.md` — 另一Janus MXene，被本文用来佐证3d能级分离机制的普适性

- **关键图表**：
  - ![图1 不同表面钝化下Mn2N的晶体结构与DOS](../../raw/figures/chen3dLevelSymmetry2025/fig_1_FPIYZWQV.png)
...
- **组织与用词**：论文按"现象观察（图1 DOS）→ 排他性分析（图2 轨道简并未变）→ 机制建立（图3 PBAND+静电势+ELF+离子模型）→ 调控验证（图4掺杂、图5应变）"的递进逻辑组织，先证伪局域配位对称性解释，再立论3d能级分离机制，最后展示该机制的预测能力。值得复用的术语：
  - 3d-level symmetry / 3d能级对称性（两侧金属层3d轨道的能级对齐关系）
  - Janus structure / Janus结构（不对称双面钝化）
  - energy-level separation / 能级分离（两侧Mn 3d轨道的相对能量差）
  - half-metallicity / 半金属性（单自旋通道导电）
...
- **可写入wiki的要点**：
  ...
  3. 所有结构中Mn的3d轨道在C3v三角晶场下均分裂为单态dz2 + 双重态eI(dyz+dxz) + 双重态eII(dxy+dx²-y²)，简并模式不随钝化原子改变——局域配位对称性不是决定半金属/半导体的主因。
  4. 核心机制：两侧Mn的3d能级分离度决定Mn→N的电荷转移量和Mn 3d电子构型。能级分离小（O/O、O/Cl）时两边均为Mn3.5+（d3.5），最高eI轨道部分填充→半金属；能级分离大（O/F、O/OH）时F侧Mn为Mn2+高自旋d5（eI全满）、O侧Mn为Mn4+低自旋d3（eI全空），费米能级落在满/空eI之间→半导体。
  ...
  10. 意义：提出"金属层间3d能级对称性"概念统一MXenes电子性质图像，为在单一材料内通过局域掺杂/应变实时"写入/擦除"半金属导电通道、构建无化学界面的可重构自旋电子器件提供理论蓝图。
