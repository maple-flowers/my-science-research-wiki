...
"主要研究对象": >-
  9.4°转角的扭曲双层石墨烯（tBLG），其VHS能量间隔约1.84 eV，与660 nm激光（1.88 eV）和金纳米盘局域表面等离子体共振（LSPR）峰精确对齐。器件由SiO₂/Si基底上的tBLG、周期274 nm、厚50 nm的金纳米盘阵列、四面体DNA折纸定位的金纳米颗粒（AuNPs）以及CRISPR-Cas12a系统组成。选择9.4°大转角还可避免小转角（<5°）常见的制备缺陷。
"主要研究方法": >-
...
- **元数据**：Bowen Du, Xilin Tian, Zhi Chen, Yanqi Ge, Chuanghu Chen, Haiyan Gao, Zhongyang Liu, Jungchen Tung, Dror Fixler, Songrui Wei, Shi Chen, Han Zhang，2025，National Science Review, 12(10): nwaf357，DOI: 10.1093/nsr/nwaf357
- **一句话**：将9.4°扭曲双层石墨烯的范霍夫奇点吸收与金纳米盘等离激元共振精确耦合，并通过DNA折纸-CRISPR-Cas12a系统动态调制局域介电环境，在60 μW低光照下实现了44.63 aM的免扩增核酸检测。
- **现有wiki双链**：
...
- **新概念/实体建议**：
  - `concepts/twistronics.md` — 转角电子学：通过旋转二维材料层间角度调控莫尔超晶格电子性质的领域
  - `concepts/van-hove-singularity.md` — 范霍夫奇点：tBLG能带中狄拉克锥交叉形成的态密度鞍点，能量由E_vhs = E₀|sin(3θ)|描述
  - `concepts/exciton-plasmon-coupling.md` — 激子-等离激元耦合：二维材料激子与金属纳米结构等离激元在能量和空间匹配时的强耦合模式
  - `concepts/crispr-cas12a.md` — CRISPR-Cas12a反式切割机制及其在生物传感中的信号放大应用
  - `concepts/dna-origami.md` — DNA折纸术：利用Watson-Crick碱基配对自组装精确纳米结构的技术
  - `concepts/dielectric-response.md` — 介电响应（含面内/面外各向异性），tBLG转角依赖的εXX/εYY与光电性能的关联
  - `entities/twisted-bilayer-graphene.md` — 扭曲双层石墨烯（tBLG），核心材料实体
  - `entities/gold-nanodisks.md` — 金纳米盘阵列，等离激元纳米天线
- **关键图表**：
  ...
  - ![图4 转角依赖的光响应度-光子能量关系、偏压调制、调制深度与FDTD热点模拟](../../raw/figures/duUltrasensitiveOptoelectronicBiosensor2025/fig_4_9UN4Q757.png)
...
- **组织与用词**：文章采用"器件设计构建 → 物理机制验证 → 光电性能表征 → 生物传感验证 → 临床转化"的经典递进结构。先建立tBLG/金纳米盘的物理增强层，再叠加DNA折纸-CRISPR生物识别层，层层验证。值得复用的术语：
  - twistronics / 转角电子学
  - van Hove singularity (VHS) / 范霍夫奇点
  - moiré superlattice / 莫尔超晶格
  - exciton–plasmon coupling / 激子-等离激元耦合
...
- **可写入wiki的要点**：
  ...
  2. 金纳米盘/tBLG异质结在60 μW低光照下光响应度达14.64 mA/W，是纯tBLG（2.34 mA/W）的6.27倍，EQE 27.51%，光电流密度为纯tBLG的7倍。金纳米盘参数：周期274 nm，厚度50 nm（Cr/Au 5/45 nm），异质结面积7 μm × 7 μm。
  ...
  4. DFPT计算表明9.4° tBLG面内介电常数εXX/εYY比13.2°和21.8° tBLG高数个数量级，面外εZZ在所有转角下均低。这种强各向异性意味着电场仅通过面内极化有效调制，9.4°的高ε增强了界面电荷屏蔽和电场局域，放大等离激元热点。
  ...
  9. 临床验证：10例肺癌患者血浆样本中miRNA-21和miRNA-155检测结果与qPCR高度一致，且本传感器信号变化率显著高于qPCR的Ct值区分度，展示了更高的信号对比度。
