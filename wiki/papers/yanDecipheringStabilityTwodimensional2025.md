...
## 📊 关键图表
笔记未附图片（raw/figures 目录下仅有 manifest.json，无图像文件）。论文原图为：图1 二维 III-V 结构演化时间线（平面蜂窝→屈曲→四方→V形→扶手椅→Haeckelite→DLHC→NB）；图2 DBSCAN 聚类得到的 III 中心/V中心积木分布；图3 LEGO/Pop-It 组装 Z13/Z33/A21、E2/E4、TT 示意图；图4(a) 各结构相对总能对比、(b) GaAs 线性回归 R²=0.95；图5 TT 相载流子迁移率及形变势贡献。
