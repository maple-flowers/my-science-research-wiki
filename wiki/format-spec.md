...
- [[../papers/<citekey>]]：<必备，一句话说明该论文对本条目贡献了什么>
- [[../papers/<citekey>]]：<必备；>15 篇的枢纽页改用分组总述式，见下方写作要求>
...
- **`📚 相关论文` 是必备小节，且每条链接后必须有贡献句**：列本条目综合过的论文，做有序去重并集。贡献句写「该论文对**本页**贡献了什么」（提出了哪个机制、给了哪个数值、验证了哪个判据），不是复述论文标题或论文类型——`- [[../papers/x]] — English Title` 与 `从综述角度梳理了「标题」` 这类都不合格。视论文数量分两种写法：
  - **≤15 篇（常规条目）**：逐条 1:1 写贡献句。
    ```markdown
    - [[../papers/<citekey>]]：<该论文对本页的具体贡献，一句话>
    ```
  - **>15 篇（枢纽页，如 2d-materials / VASP / spin-orbit-coupling）**：改为**按主题分组总述式**，避免为「只是用到了本概念」的论文硬凑贡献句。每组一句总述说明该组论文共同确立了什么，组内一行一链；另挑 5–10 篇真正代表性的论文单独写贡献句放在「代表性工作」子块。
    ```markdown
    ### 代表性工作
    - [[../papers/<citekey>]]：<单独的具体贡献句>

    ### <主题分组名>（<N> 篇）
    这些工作共同<确立/验证/给出>了……
    - [[../papers/<citekey>]]
    ```
    分组式下组内链接不要求逐条贡献句，但每个分组必须有总述句；分组依据写在组名里，不得用「其他」兜底超过 5 篇。
...
- 不要保留 Zotero 元数据表、AI 转写、转换日志等噪音；页尾**不得**残留 `*（内容由AI生成，仅供参考）*` 这类外部工具声明，frontmatter 也不得保留 `AIGC:` 水印块（由 `tools/audit_wiki_pages.py --strict` 拦截）。
