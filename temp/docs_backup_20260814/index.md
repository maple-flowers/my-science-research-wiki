...
*共 103 个正式概念页（反链聚合中间产物不计入），完整列表见 `wiki/concepts/` 目录。*
...
*共 22 个正式实体页（反链聚合中间产物不计入），完整列表见 `wiki/entities/` 目录。*
...
- [[wiki/projects/index|科研项目总索引 Projects Overview]]
  - [[wiki/projects/project-1-two-photon|项目一：双光固化和双光发光]]
...
*文献鸟（Stork）自动推送写入 `raw/文献日报/`，由 [[SCHEMA#摄入源|摄入源规范]] 与 `stork_daily.py` 维护（该脚本已随 `tools/` 清空、待重建）。当前目录暂无已归档日报。*

## 图表库（Figures）

系统分类索引见 [[wiki/figures/_index|图表库总索引]]（25 页 / 1080 个条目）。核心分类枢纽页：
