---
name: format-spec
description: 六类条目的编写格式规范（图表库 / 概念与实体 / 论文条目 / 写作库 / 主题 / 项目）——怎么写；踩坑经验不在此页
---

# 条目编写规范 (Format Specifications)

> 本文件定义各层条目的编写格式规范（2026-08-12 自 [[SCHEMA]] 拆分归档）。规范总纲（目录约定 / 标签体系 / 链接规范 / 铁律）见 [[SCHEMA]]，批量维护的踩坑经验与时间记录见 [[log]]。
